...
impl WriteBatch {
    ...
    pub fn put_cf<K, V>(&mut self, cf: impl ColumnFamilyRef, key: K, value: V)
    where
        K: AsRef<[u8]>,
        V: AsRef<[u8]>,
    {
    ...
    pub fn merge_cf<K, V>(&mut self, cf: impl ColumnFamilyRef, key: K, value: V)
    where
        K: AsRef<[u8]>,
        V: AsRef<[u8]>,
    {
    ...
    pub fn delete_cf<K: AsRef<[u8]>>(&mut self, cf: impl ColumnFamilyRef, key: K) {
        let key = key.as_ref();
    ...
    /// including "begin_key" and excluding "end_key". It is not an error if no
    /// keys exist in the range ["begin_key", "end_key").
    pub fn delete_range_cf<K: AsRef<[u8]>>(&mut self, cf: impl ColumnFamilyRef, from: K, to: K) {
        let (start_key, end_key) = (from.as_ref(), to.as_ref());
